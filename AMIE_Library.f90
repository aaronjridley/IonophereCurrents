...
subroutine AMIE_GetHall(TimeIn, Method, &
     IEi_nMLTs, IEi_nLats, IEi_nBLKs, HallOut, iError)
  ...
  real*8, intent(in) :: TimeIn
  integer, intent(in) :: Method, IEi_nMLTs, IEi_nLats, IEi_nBLKs
  real, dimension(IEi_nMLTs,IEi_nLats, IEi_nBLKs), intent(out) :: HallOut
  real, dimension(IEi_nMLTs,IEi_nLats, IEi_nBLKs)              :: ValueOut
  integer, intent(out) :: iError

  call AMIE_GetValue(TimeIn, Method, hall_, &
       IEi_nMLTs, IEi_nLats, IEi_nBLKs, ValueOut, iError)

  if (iError /= 0) then
     write(*,*) "Error in routine AMIE_GetHall:"
     write(*,*) cErrorCodes(iError)
     stop
  else
     HallOut = ValueOut
...
end subroutine AMIE_GetHall

!----------------------------------------------------------------------

subroutine AMIE_GetPed(TimeIn, Method, &
     IEi_nMLTs, IEi_nLats, IEi_nBLKs, PedOut, iError)
  ...
  real*8, intent(in) :: TimeIn
  integer, intent(in) :: Method, IEi_nMLTs, IEi_nLats, IEi_nBLKs
  real, dimension(IEi_nMLTs,IEi_nLats, IEi_nBLKs), intent(out) :: PedOut
  real, dimension(IEi_nMLTs,IEi_nLats, IEi_nBLKs)              :: ValueOut
  integer, intent(out) :: iError

  call AMIE_GetValue(TimeIn, Method, Ped_, &
       IEi_nMLTs, IEi_nLats, IEi_nBLKs, ValueOut, iError)

  if (iError /= 0) then
     write(*,*) "Error in routine AMIE_GetPed:"
     write(*,*) cErrorCodes(iError)
     stop
  else
     PedOut = ValueOut
  ...
  do iBLK = AMIE_South_, AMIE_North_
     ...
     if (Method == AMIE_After_) then
        if (iBLK == AMIE_South_) then
           ...
           if (iValue == hall_) then
              ValueOut(1:AMIE_nMLTs, 1:AMIE_nLats,iBLK) =  &
                   AMIE_Hall(1:AMIE_nMLTs, 1:AMIE_nLats,iTime,iBLK)
           endif

           if (iValue == ped_) then
              ValueOut(1:AMIE_nMLTs, 1:AMIE_nLats,iBLK) =  &
                   AMIE_Ped(1:AMIE_nMLTs, 1:AMIE_nLats,iTime,iBLK)
           endif
           ...
           do iLat = AMIE_nLats,1,-1
              ...
              if (iValue == hall_) then
                 ValueOut(1:AMIE_nMLTs, iLat,iBLK) =  &
                      AMIE_Hall(1:AMIE_nMLTs, AMIE_nLats - iLat + 1,iTime,iBLK)
              endif

              if (iValue == ped_) then
                 ValueOut(1:AMIE_nMLTs, iLat,iBLK) =  &
                      AMIE_Ped(1:AMIE_nMLTs, AMIE_nLats - iLat + 1,iTime,iBLK)
              endif
     ...
     if (Method == AMIE_Interpolate_) then
        ...
        if (iBLK == AMIE_South_) then
           ...
           if (iValue == ped_) then
              ValueOut(1:AMIE_nMLTs, 1:AMIE_nLats,iBLK) =  &
                   (1.0 - dt)*AMIE_Ped(1:AMIE_nMLTs, 1:AMIE_nLats,iTime,iBLK)+&
                           dt*AMIE_Ped(1:AMIE_nMLTs, 1:AMIE_nLats,iTime-1,iBLK)
           endif
           if (iValue == hall_) then
              ValueOut(1:AMIE_nMLTs, 1:AMIE_nLats,iBLK) =  &
                   (1.0 - dt)*AMIE_Hall(1:AMIE_nMLTs, 1:AMIE_nLats,iTime,iBLK)+&
                           dt*AMIE_Hall(1:AMIE_nMLTs, 1:AMIE_nLats,iTime-1,iBLK)
           endif
        else
           ...
           do iLat = AMIE_nLats,1,-1
              ...
              if (iValue == hall_) then
                 ValueOut(1:AMIE_nMLTs, iLat,iBLK) =  &
                      (1.0 - dt)*AMIE_Hall(1:AMIE_nMLTs,AMIE_nLats-iLat+1,&
                                         iTime,iBLK) + &
                     dt*AMIE_Hall(1:AMIE_nMLTs, AMIE_nLats-iLat+1,iTime-1,iBLK)
              endif
              if (iValue == ped_) then
                 ValueOut(1:AMIE_nMLTs, iLat,iBLK) =  &
                      (1.0 - dt)*AMIE_Ped(1:AMIE_nMLTs,AMIE_nLats-iLat+1,&
                                         iTime,iBLK) + &
                     dt*AMIE_Ped(1:AMIE_nMLTs, AMIE_nLats-iLat+1,iTime-1,iBLK)
              endif
           enddo
        endif
     endif
...
subroutine get_AMIE_values(rtime)
  ...
  call AMIE_GetHall(rtime, EIE_Closest_, &
       EIEi_HavenMlts, EIEi_HavenLats, EIEi_HavenBLKs, EIEr3_HaveHall, iError)

  call AMIE_GetPed(rtime, EIE_Closest_, &
       EIEi_HavenMlts, EIEi_HavenLats, EIEi_HavenBLKs, EIEr3_HavePed, iError)
