...
!  endif
  ...
  if (index(EIE_NameOfEFieldModel,'amie') > 0) then
     ...
     if (iDebugLevel > 3) write(*,*) "====> Reading north AMIE file: ",AMIEFileNorth
     call AMIE_SetFileName(AMIEFileNorth)
     call readAMIEOutput(North_, .false., iError)

     if (iDebugLevel > 3) write(*,*) "====> Reading south AMIE file: ",AMIEFileSouth
...
subroutine EIE_InitGrid(nLats, nMlts, nBlocks, iOutputError)
  ...
  allocate(EIEr3_HaveHall(EIEi_HavenMlts,EIEi_HavenLats,EIEi_HavenBLKs), &
       stat=iError)
  if (iError /= 0) then
     write(*,*) "Error in allocating array EIEr3_HaveHall in Interface"
     stop
  endif

  allocate(EIEr3_HavePed(EIEi_HavenMlts,EIEi_HavenLats,EIEi_HavenBLKs), &
       stat=iError)
  if (iError /= 0) then
     write(*,*) "Error in allocating array EIEr3_HavePed in Interface"
     stop
  endif
