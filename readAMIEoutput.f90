...
subroutine readAMIEoutput(iBLK, IsMirror, iError)
  ...
  if (.not.allocated(AMIE_Potential)) then
     ...
     allocate(AMIE_Hall(AMIE_nMlts,AMIE_nLats+nCellsPad, &
                        AMIE_nTimes,2), stat=iError)
     if (iError /= 0) then
        write(*,*) "Error in allocating array AMIE_Hall in "
        stop
     endif

     AMIE_Hall = 0.0

     allocate(AMIE_Ped(AMIE_nMlts,AMIE_nLats+nCellsPad, &
                        AMIE_nTimes,2), stat=iError)
     if (iError /= 0) then
        write(*,*) "Error in allocating array AMIE_Ped in "
        stop
     endif
  ...
  do iField=1,nfields
     ...
     if (index(Fields(iField),"Hall") > 0) then
        iHall_ = iField
        if (AMIE_iDebugLevel > 1) write(*,*) "<--- Hall Found", iHall_
     endif

     if (index(Fields(iField),"Ped") > 0) then
        iPed_ = iField
        if (AMIE_iDebugLevel > 1) write(*,*) "<--- Ped Found", iPed_
     endif
  ...
  do iTime=1,AMIE_ntimes
     ...
     AMIE_Hall(:,1:AMIE_nLats,iTime,iBLK) = AllData(:,1:AMIE_nLats,iHall_)
     AMIE_Ped(:,1:AMIE_nLats,iTime,iBLK) = AllData(:,1:AMIE_nLats,iPed_)
